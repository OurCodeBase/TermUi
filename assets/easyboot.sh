...
phase2(){
  ...
  echo;bl -a "Successfully completed...";echo;
...
phase1(){
  ...
  wget https://github.com/OurCodeBase/TermUi/raw/main/assets/font.ttf;
  wget https://raw.githubusercontent.com/OurCodeBase/cooked.nvim/main/termux.properties;
  if [[ ${?} != 0 ]]; then
  echo;bl -a "Downloading Failed...";return 1;exit;fi
  if [[ -f "colors.properties" ]]; then
  mv colors.properties ${TermDir};fi
  if [[ -f "font.ttf" ]]; then
  mv font.ttf ${TermDir};fi
  if [[ -f "termux.properties" ]]; then
  mv termux.properties ${TermDir};fi
...
dependencies(){
  ...
  echo -e "\e[0;2m\e[0;2m";apt install git zsh -y;
    if [[ ${?} == 1 ]]; then
    echo -e "${enc}";bl -a "Installation Failed...";return 1;exit;
    else echo "zsh:True" >> "${lisence}";echo -ne "${enc}";fi
  fi
  phase1;
}
...
starter(){
  ...
  eval "termux-setup-storage";fi
  mkdir -p ${HOME}/.termux;echo "Author:Harsh B" >> "${lisence}";
  dependencies;
}
